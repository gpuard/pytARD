...
# dp = DampingProfile(room_width, c, 0.9)
...
partitions.append(PMLPartition2D(np.array([
    [1.5],        # X, width
    [room_width], # Y, depth
...
if TEST_KIND == 'X':
    ...
    axis=0
    interfaces.append(InterfaceData2D(0, 1, Direction2D.Y))
    title = 'X-PML-WALL'
elif TEST_KIND == 'all':
    pass
...
if visualize:
    ...
    if a == 0:
        ...
        fps=30
        anim = AnimationPlotter().plot_2D(pf_t, 
                                          sim_param,
                                          title,
                                          interval = 1000 / fps, # in ms
                                          yx=partitions[0].src_grid_loc)
    else:
        plotter = Plotter()
